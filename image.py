...
def load_data(img_path,train = True):
    ...
    prev_img = prev_img.resize((1280,720))
    img = img.resize((1280,720))
    post_img = post_img.resize((1280,720))

    gt_file = h5py.File(gt_path)
    target = np.asarray(gt_file['density'])
    gt_file.close()
    target = cv2.resize(target,(int(target.shape[1]),int(target.shape[0])),interpolation = cv2.INTER_CUBIC)*64 # change here

    prev_gt_file = h5py.File(prev_gt_path)
    prev_target = np.asarray(prev_gt_file['density'])
    prev_gt_file.close()
    prev_target = cv2.resize(prev_target,(int(prev_target.shape[1]),int(prev_target.shape[0])),interpolation = cv2.INTER_CUBIC)*64 #change /8

    post_gt_file = h5py.File(post_gt_path)
    post_target = np.asarray(post_gt_file['density'])
    post_gt_file.close()
    post_target = cv2.resize(post_target,(int(post_target.shape[1]),int(post_target.shape[0])),interpolation = cv2.INTER_CUBIC)*64 #change
