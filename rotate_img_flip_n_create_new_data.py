...
def create_flip_image(img_path, img_json):
    ...
    flipped_image = cv2.rotate(rotated_image, cv2.ROTATE_90_COUNTERCLOCKWISE)
    print("image rotated")
    for project_data in jsonFile["projects"].keys():            
        ...
        for object in objects:
            ...
            object["bounding_box"] = rotate90Deg(object["bounding_box"], height)

            print("bounding box modifying...")
            # time.sleep(1)
        
        with open(flip_path(img_json), 'w') as json_file:
    ...
    cv2.imwrite(flip_path(img_path), flipped_image)
...
def flip_path(path):
    ...
    return (path.rsplit('.', 1)[0] + '_f' + '.' + postfix) 
...
for root,dirs, files in os.walk(root_data):
    for file_name in files:
        if file_name.endswith('.json'):
            ...
            create_flip_image(img_path, img_json)
