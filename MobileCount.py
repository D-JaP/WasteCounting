...
class MobileCount(nn.Module):
    ...
    def forward(self, x):
        ...
        l4 = self.dropout4(l4)
        x4 = self.p_ims1d2_outl1_dimred(l4) # conv 1x1 (254,64)
        x4 = self.relu(x4) 
        x4 = self.mflow_conv_g1_pool(x4) # CRP
        x4 = self.mflow_conv_g1_b3_joint_varout_dimred(x4) # conv 1x1 (64,32)
        x4 = nn.Upsample(size=l3.size()[2:], mode='bilinear')(x4) #change size
        ...
        l2 = self.p_ims1d2_outl3_dimred(l2)
        x2 = self.adapt_stage3_b2_joint_varout_dimred(l2)
        x2 = x2 + x3
        x2 = F.relu(x2)
        ...
        l1 = self.p_ims1d2_outl4_dimred(l1)
        x1 = self.adapt_stage4_b2_joint_varout_dimred(l1)
        x1 = x1 + x2
        x1 = F.relu(x1)
